...
> **配套**：音频连贯性详见 [skill_audio.md](./skill_audio.md)（reference_audios 共享 / MV 模式 / 4 类特殊字符）
...
详细调度策略见 [skill_async_schedule.md](./skill_async_schedule.md)。

---

## 配乐策略（搭配链式衔接的关键）

> **完整音频规范见 [skill_audio.md](./skill_audio.md)。** 这里只列项目级铁律。
...
- ✅ 正确做法：
  ...
  - 失败时**先诊断不要降级**：读错误 → 排查参数（详见 [skill_self_decision.md](./skill_self_decision.md) "失败不许降级"）
...
- ✅ 正确做法：part2 必须传上段 url；**Seedance 只出对白+音效不出 BGM**，整片拼好后再统一 `audio_amix` 铺一首 BGM（详见 [skill_audio.md](./skill_audio.md)）
