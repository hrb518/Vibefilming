...
> **关键工具**：`storyboard_set` / `storyboard_get`

---

## 铁律：分镜先行

`project_create` 之后**第一件事不是出图**，是 `storyboard_set` 把分镜落盘。
- 单镜头视频可省，**≥2 镜头必做**
- 没分镜直接拍 → entity 漏登记 / 镜头堆砌不连贯 / 预算失控（前几镜成废片）
...
```json
{
  ...
  "entities_planned": [
    {"name": "protagonist_li", "type": "character",
     ...
     "views_needed": ["turnaround"]},   // 默认 1 张三视图（front/side/back 合一）就够
...
**写在思考的 `<self_review>` 块里**，不通过就再次 `storyboard_set` 覆盖更新（最多 2 轮，第 3 轮直接走下去）。

| # | 标准 | 怎么查 |
|---|---|---|
| 1 | 节奏配比 | 单镜 5-8s（最长 15s）；总时长按 brief；开头/高潮/收尾节奏对吗？ |
| 2 | 衔接合理性 | `chain_with` 链下来动作连得上吗？有没有突然切到无关画面？ |
| 3 | entity 完整性 | shots 里出现的所有 character/prop/scene 都在 `entities_planned` 里吗？ |
| 4 | 镜头多样性 | camera 全是"中景静态"？混入推/拉/特写/俯拍/横移 |
| 5 | 预算估算 | `len(shots) × 1.2 重做率` 不超过 `max_seedance_calls` 才安全 |
| **6** | **entities_planned 非空** ⭐ | **任何真实视频项目（不是抽象概念图测试）都必须列出至少 1 个 entity。空列表直接打回重写**——已踩坑：p20260531_140047_ 环保广告 entities_planned 为空，agent 跳过 entity 流程靠文字脑补人物，第一次出年轻人不符合"中年男子" |

---

## storyboard_get 的典型用法

- 出每个 shot 之前先调一次，看 action / camera / characters
...
- entity 漏登记 → 拍到一半发现某角色没建档，回头补 → 风格会跳
...
详见 [skill_self_decision.md](./skill_self_decision.md) "失败处置：不许偷偷降级"。
...
> 详细规范见 [skill_audio.md](./skill_audio.md)。
