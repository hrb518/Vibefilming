...
def main():
        ...
        try:
            ...
            try:
                if data["data"][0].get("url"):
                    ...
                    p = save_bytes(f"seedream_{m}.png", img)
                    info(f"已保存 → {p}")
    ...
    fail(f"所有候选 model 均失败：{last_err}")
    info("可能原因：账号未开通该模型 / model id 名称已变更（去 Ark 控制台-模型广场确认）")
    return False
